...
- 高画質は最大256色の適応型パレットを再調整し、弱い誤差拡散で階調を補正

GIF形式は1枚の画像で最大256色、透明度は完全な透明か不透明の2段階です。そのため、元画像と色や半透明の縁を完全一致させることはGIFの仕様上できません。このツールでは仕様上必要な減色以外の色ずれを抑えます。色の正確さを最優先する静止画像はPNG、容量とのバランスを取る場合はWebPを使用してください。
...
最初は360px、3秒、10fps、色品質は標準で試します。標準は色味と処理速度のバランスを優先します。グラデーションの段差が気になる場合だけ高画質を使用します。動作が重い場合は256px、8fps、色品質は軽量へ下げます。
...
さらに、肌色、グラデーション、彩度の異なる色を含む検証画像を減色し、平均色差とRGB各チャンネルの偏りが上限内であることを確認します。色変換テストは64MBのNode.jsヒープでも実行できます。

GitHub Actionsでは同じ検査をPull Requestとmain更新時に実行します。公開サイトは毎回のデプロイ後と毎週確認し、公開中のGIFエンコーダーを取得して同じ復号テストと色再現テストを実行します。
