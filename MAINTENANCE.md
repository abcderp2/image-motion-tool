...
- 「動きの種類」は9種類を一覧表示し、現在の項目を強調したうえで、1つのボタンを押すたびに指定順で循環する。「動きの速さ」はプレビューとGIF、APNG、アニメーションWebPの再生速度へ反映し、アニメーション内の動作回数は通常設定で指定できる状態を保つ
...
- 別タブ用リンクにはtarget="_blank"とrel="noopener noreferrer"を維持し、別タブ内の画像は画面中央へ配置する
...
4. 9種類の動きが一覧表示され、現在の項目が強調されることを確認してから、動きの種類ボタンを押して指定順の9種類を循環させながら再生する
...
8. 画像を選んだ後に現在のプレビューを別タブで開き、画面中央に表示されることを確認する
