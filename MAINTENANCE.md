...
6. 取り消しとやり直しを確認する
7. 4種類の画面比率を確認する
8. 透明背景で移動量と回転量を大きくし、軽量、標準、高画質のGIFを保存する
9. 保存した透明GIFを別画面で開き、過去の位置が残らず、色がフレームごとに濃くならないことを確認する
10. 白背景でも標準と高画質のGIFを保存し、元画像と並べて肌色、白、黒、グレー、青、赤を確認する
11. PNG、JPEG、WebPを保存する
12. 設定JSONを書き出し、読み戻す
13. 画像を外した後も設定だけが残ることを確認する
14. オフラインで再読み込みする
15. 画面を拡大しても操作不能にならないことを確認する
...
3. Live site checkが公開中のgif-encoder.jsを取得し、LZW復号、色再現、透明残像テストまで成功したことを確認する
4. 公開URLを通常ウィンドウで開き、画面下部のBuild番号を確認する
5. Service Worker更新後にページを再読み込みする
6. 透明背景で標準と高画質のGIFを保存する
7. 保存したGIFを新しいタブで開き、過去位置の残像と色の重なりがないことを確認する
8. 背景色ありのGIFと静止PNGを並べ、仕様上の減色を超える色かぶりがないことを確認する
9. オフラインで再読み込みし、更新後の資産が使われることを確認する
10. 問題がなければPull Requestを閉じた状態で残す
...
- GIF変更ではLZW復号、色差、透明フレーム残像の検査を省略しない条件
