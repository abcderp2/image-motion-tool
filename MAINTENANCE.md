...
6. 左右に傾くが足元を支点にし、横移動だけに見えないことを確認する
7. 呼吸するが足元を保ち、縦横へ穏やかに伸縮することを確認する
8. ゆっくり拡大して戻るが等倍から始まり、拡大後に等倍へ戻ることを確認する
9. 振り子が画像上部を支点にし、左右に傾くと異なる見え方になることを確認する
10. 指ドラッグ、矢印キー、左右反転を確認する
11. 取り消しとやり直しを確認する
12. 4種類の画面比率を確認する
13. 白背景が大部分を占める横長画像を透明背景の正方形へ配置し、標準と高画質のGIFを保存する
14. 保存GIFが黒いシルエットへ崩れず、白、肌色、青などの色が残ることを確認する
15. 透明背景で移動量と回転量を大きくし、軽量、標準、高画質のGIFを保存する
16. 保存後に別タブ用リンクが表示され、生成したGIFそのものが新しいタブで開くことを確認する
17. 別タブでピンチ操作またはブラウザの拡大機能を使い、GIFが動いたまま拡大できることを確認する
18. 2回目のGIF生成後も最新のGIFだけが開き、古い一時URLが残らないことを確認する
19. 保存した透明GIFを別画面で開き、過去の位置が残らず、色がフレームごとに濃くならないことを確認する
20. PNG、JPEG、WebPを保存する
21. 設定JSONを書き出し、読み戻す
22. 画像を外した後も設定だけが残ることを確認する
23. オフラインで再読み込みする
24. 画面を拡大しても操作不能にならないことを確認する
