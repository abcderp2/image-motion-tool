...
- 別タブ表示用のGIFまたはAPNGは出力種類ごとに生成済みBlobから一時URLを1つだけ作り、次の生成時とpagehide時に破棄する
...
robots.txtとai.txtは公開方針であり、認証やアクセス制御ではありません。端末内処理でも、壊れた画像や極端に大きい画像がブラウザの画像デコーダーへ負荷を与える可能性は残ります。そのため、ヘッダー確認と端末別上限を画像展開前に実施します。GIF速度変更は画像デコーダーやCanvasを通さず、15MB、8192px、4096フレームの上限と構造確認を通過したバイト列だけを処理します。APNG生成はフレームを1枚ずつCanvas PNGへ変換し、14メガ画素、出力32MB、推定メモリ64MBを通常上限として、2GB以下の端末ではさらに下げます。
