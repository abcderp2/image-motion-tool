...
- APNGの各Canvas PNGフレームでPNGシグネチャ、チャンク長、チャンク名、CRC、8ビットRGBAのIHDR、IDAT、IENDを確認する
- APNG生成前にフレーム数、画像寸法、総処理画素数、推定出力サイズ、推定メモリを制限し、低メモリ端末ではより厳しい上限を使う
- APNG出力はacTL、fcTL、IDAT、fdAT、IENDの順序とCRCを自前で確認し、入力フレームの不一致や上限超過を拒否する
- アニメーションWebPの各CanvasフレームでRIFFのチャンク名、チャンク長、データ境界、WEBP、VP8X、VP8、VP8L、ALPHの実チャンクを確認し、ANIM、ANMFの寸法、表示時間、予約ビット、フレーム数、ループを検査する
- アニメーションWebPはCanvas標準の非可逆出力だけを使い、可逆圧縮を実装したように扱わない。追加エンコーダーを導入する場合は別途ライセンス、供給元、脆弱性確認を行う
...
- 別タブ表示用のGIF、APNGまたはアニメーションWebPは出力種類ごとに生成済みBlobから一時URLを1つだけ作り、次の生成時とpagehide時に破棄する
...
robots.txtとai.txtは公開方針であり、認証やアクセス制御ではありません。端末内処理でも、壊れた画像や極端に大きい画像がブラウザの画像デコーダーへ負荷を与える可能性は残ります。そのため、ヘッダー確認と端末別上限を画像展開前に実施します。GIF速度変更は画像デコーダーやCanvasを通さず、15MB、8192px、4096フレームの上限と構造確認を通過したバイト列だけを処理します。APNG生成はフレームを1枚ずつCanvas PNGへ変換し、14メガ画素、出力32MB、推定メモリ64MBを通常上限として、2GB以下の端末ではさらに下げます。アニメーションWebPは14メガ画素、出力24MB、推定メモリ64MBを通常上限として、2GB以下の端末ではさらに下げます。

Blob URLは同じブラウザ内で生成物を表示するための一時的な参照です。外部送信ではありませんが、URLを保持している間はそのブラウザ内でGIFまたはAPNGへアクセスできます。このため、通常生成と速度変更後の各出力種類で同時保持を1件に制限し、更新時とページ離脱時に明示的に破棄します。速度変更用の元GIF、APNGフレームPNG、生成Blobはメモリ上だけに保持し、Service Workerの許可一覧にはBlob URLや利用者のGIFを含めません。
